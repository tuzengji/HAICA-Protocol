...
这个目录是一个按 [HAICA Protocol V1.1](../../HAICA-Protocol.md)编写的示例任务包，演示如何让一道题同时接收报告、数据表和图片，并用确定性检查与模型判卷共同评分。示例数据是为了演示协议而构造的，不代表真实业务。
...
本示例声明 `protocol_version = "1.1"`，须由支持 V1.1 的平台导入。
